...
curl -sS -H 'Authorization: Bearer local-admin-key' \
  "http://127.0.0.1:8082/api/v1/admin/subscriptions?user_id=$USER_ID&limit=10" | python3 -m json.tool
...
`/api/v1/admin/subscriptions` is the subscription-side companion read model. It can filter by `user_id`, `sku_code`, Walnut projection `status`, `provider`, or `out_trade_no`, and returns the same `SoftwareSubscriptionProjector` status used by client snapshots plus cancel/resume flags, active fulfillment grants, latest order summary, provider-control status metadata, cancellation facts, and payment-event `payload_hash`. It deliberately does not return raw email, checkout URL, provider customer id, provider subscription id, provider event id, webhook raw payload, or idempotency keys.
...
export ADMIN_PRINCIPALS_JSON='[{"name":"support","key":"support-key","permissions":["admin.dashboard.read","admin.access_accounts.read","admin.users.read","admin.orders.read","admin.subscriptions.read","admin.cloud_storage.read","admin.audit.read"]},{"name":"ops","key":"ops-key","permissions":["admin.*"]}]'
```
...
scripts/verify_admin_subscription_contract.sh
...
- When `SERVER_ENV=prod`, startup validates Creem live credentials, webhook secret, full product map, rate limit, admin auth, Ed25519 snapshot signer, versioned database migration mode, HTTPS CORS origins, security headers, and checkout redirect allowlist before registering providers.
