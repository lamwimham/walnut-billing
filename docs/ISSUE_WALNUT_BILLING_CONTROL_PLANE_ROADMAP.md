...
状态：Control-plane Closed / Live external credential validation pending
适用仓库：`walnut-billing`  
更新日期：2026-06-20
...
| 云存储 | App 直传对象存储，billing 只管配额、manifest、metadata | `CloudProject + CloudManifest + CloudObject + CloudSyncSession` | P1 |
...
| 不把对象存储凭证或对象正文暴露给 Walnut App | App 只接收短期 upload/download target；billing 只保存 metadata |
...
| 能力 | 当前代码位置 | 评价 |
|---|---|---|
| 服务启动、DI、路由挂载 | `cmd/server/main.go`, `internal/app/bootstrap` | 入口已缩减为启动/优雅关闭，DI、migration、provider、route registrars 已迁入 bootstrap |
| 配置 | `internal/config/config.go` | 已覆盖 access、Creem、checkout、renewal、cloudstorage、HTTP；prod fail-fast 校验已分层 |
...
| 管理后台 | `internal/api/handler/*admin*`, `internal/web/static/index.html` | 已有 API key/RBAC、脱敏账号、订单/订阅/云存储 read models、dashboard shell、dev/test reset facade |
| 可重复测试环境 | `scripts/run_deterministic_billing.sh`, `docs/LOCAL_COMMERCE_TEST_ENV.md` | 已具备 deterministic mock profile、Creem sandbox runbook、webhook fixtures、dev/test scenario reset |
...
| Cloud storage provider 曾未定 | 过早实现容易变成假代码 | 已通过 ADR 落地 S3-compatible/R2 adapter；provider 仍在 port 后面 |
| Live 外部集成依赖真实凭证 | 本地无法证明 dashboard 付款、真实 webhook 和对象存储账号均可用 | 本地合同闭环；上线前使用真实 Creem/R2 sandbox/prod 凭证做一次 runbook 验证 |
...
| 已落地实体 | `AccessLoginChallenge`：OTP/magic-link challenge，存 hash、过期时间、attempts、consumed_at |
...
| 已落地投影 | `SoftwareSubscriptionProjector`：Walnut 自有订阅投影，不直接等于 provider subscription |
...
| 已落地实体 | `CloudSyncSession`：记录 upload session、过期、requested bytes、commit 状态 |
...
Provider ADR 已决策并落地 S3-compatible adapter，未配置 provider 时仍保留 explicit unconfigured behavior：

| Provider 选项 | 当前结论 |
|---|---|
| S3-compatible / Cloudflare R2 | 首选实现，已支持 SigV4 upload/download/head/delete target |
| 阿里云 OSS | 保留为未来 adapter，可通过 `ObjectStorageProvider` port 扩展 |
| 自建 MinIO | 可作为 S3-compatible 目标验证，但生产需额外运维 runbook |
| Supabase / managed storage | 暂不作为第一实现，避免权限模型锁定 |
...
| `access_login_challenges` | identity | 已落地，邮箱登录/恢复 OTP/magic link |
...
| `software_subscriptions` | commerce/access projection | 已以 service projector 等价落地，订阅展示和互斥策略的 read model |
...
| `cloud_sync_sessions` | cloud_storage | 已落地，上传授权会话 |
...
进展（2026-06-20）：已完成。当前增量把进程入口缩减为启动/优雅关闭，把 config、DB、migration、repository、provider、service 和路由组装迁入 `internal/app/bootstrap`；新增 module registrar 与 import guard 测试来固定边界。
...
WCP-1 进展（2026-06-20）：本地合同已完成。Creem adapter contract 已增加 sandbox/prod endpoint/key 防混用、checkout-visible SKU product map 校验、checkout/subscription-control endpoint 契约，以及本地 webhook fixture runner；真实 dashboard 支付仍需上线前使用实际 Creem sandbox/live 凭证验证。
...
- 新增 `scripts/verify_creem_sandbox_contract.sh` 或 Go integration test fixture runner。（已完成：`scripts/verify_creem_sandbox_contract.sh`）
...
WCP-4 进展（2026-06-20）：第五切片已完成。新增 dev/test-only `AdminTestScenarioResetService` + `AdminTestScenarioResetRepository` + `POST /api/v1/admin/test/scenarios/reset`，形成 deterministic 场景 reset facade。handler 强制要求认证 principal 具备 `admin.test.write`，service 再用 `ServerEnvAdminTestScenarioResetPolicy` 拒绝 `SERVER_ENV=prod`，repository 才在事务内清理单个测试账号相关的 identity/access/commerce/subscription/risk/cloud/credit facts；`dry_run=true` 只返回 per-table affected counts。响应和 audit 只返回 masked email/fingerprint、user_id、affected counts 与稳定错误 code，不返回 raw email、raw device id、provider subscription id、object key、checkout URL 或 raw webhook payload。新增 `scripts/verify_admin_test_scenario_reset_contract.sh` 与 `docs/LOCAL_COMMERCE_TEST_ENV.md` 操作步骤，固化非 prod 策略、专用权限、audit redaction、repository graph cleanup 和架构边界。
...
- 增加 dev/test-only scenario reset：只能在 `SERVER_ENV != prod` 且 `admin.test.write` 时启用。（第五切片已完成）
...
| 决策 | 当前建议 | 阻塞模块 |
|---|---|---|
| 云存储 provider | 已选 S3-compatible / Cloudflare R2，生产通过 `CLOUD_STORAGE_PROVIDER=r2|s3` 配置切换 | 已闭环；上线前用真实 bucket 凭证验证 |
...
当前代码和本地合同已达到 BYOK-first 商业授权控制面的生产级闭环；仍需在上线前使用真实 Creem/R2 凭证完成一次外部 sandbox/prod runbook 验证，证明 dashboard 支付、真实 webhook 回放和对象存储账号权限均正确。
